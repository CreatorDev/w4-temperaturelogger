...
/**
 * Build Temperaturelogger Workshop
 *
 */
...
node('docker && imgtec') {
    ...
    docker_image.inside {
        stage('Prepare') {
            checkout([$class: 'GitSCM',
                ...
                extensions: scm.extensions + [
                    ...
                    [$class: "RelativeTargetDirectory", relativeTargetDir: "${WORKSPACE}/workshop/temperature-logger"],
                ],
            ])
        ...
        stage('Build') {
          sh "make -j1 V=s package/temperature-logger/compile"
        }
        stage('Upload') {
